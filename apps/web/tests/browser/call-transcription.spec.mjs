...
// Real recorder/WebAudio with an oscillator, NOT human speech or a microphone.
...
test("slow multilingual STT is not displaced by the browser's confident English guess",async ({page,context}) => {
...
test("a consent rejection remains authoritative even when browser recognition has text",async ({page,context}) => {
  await capture(page,context);
...
test("hang-up fences a late transcription without reviving the call",async ({page,context}) => {
  await capture(page,context);
