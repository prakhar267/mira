...
const directory=new URL("../../../audit/2026-09-12-launch/",import.meta.url);await mkdir(directory,{recursive:true});await writeFile(new URL("conversation-eval.json",directory),JSON.stringify(report,null,2));console.log(JSON.stringify({...report,results:undefined},null,2));if(report.passed<report.total||!report.stableDeployment)process.exitCode=1;
