...
import { companionVoiceIdentity, companionVoiceModes } from "@/lib/voice-profiles";
        ...
        <div className="onboarding__copy">
          ...
          {step === 5 && (
            <>
              ...
              <div className="form-grid"><label className="field">Presentation<select value={draft.presentation} onChange={(event) => setDraft({ ...draft, presentation: event.target.value })}><option value="playful and warm">Playful & warm</option><option value="bright and expressive">Bright & expressive</option><option value="calm and thoughtful">Calm & thoughtful</option></select></label><label className="field">Voice mood<select value={draft.voiceId} onChange={(event) => setDraft({ ...draft, voiceId: event.target.value })}>{companionVoiceModes.map((mode) => <option key={mode.id} value={mode.id}>{`${companionVoiceIdentity.name} · ${mode.name}`}</option>)}</select></label></div>
            </>
          )}
