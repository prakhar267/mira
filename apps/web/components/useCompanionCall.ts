...
export function useCompanionCall(userName: string, onUserTurn: (text: string) => Promise<string>) {
  ...
  useEffect(() => {
    ...
    const session = new CallSession({
      ...
      respond: text => respond.current(text),
      update: setState,
    ...
    return () => { window.clearTimeout(start); window.clearInterval(timer); session.close(); if (call.current === session) call.current = null; };
  }, [greeting]);
