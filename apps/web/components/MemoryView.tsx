...
export function MemoryView({ memories, enabled, onToggle, onUpdate, onDelete, onAdd }: {
  ...
  return (
    <section className="workspace memory-view" aria-labelledby="memory-title">
      <header className="workspace-header workspace-header--text">
        <div><span className="eyebrow">Continuity you control</span><h1 id="memory-title">Memory</h1><p>{enabled ? `${visible.length} active memories. Every item stays inspectable.` : "Memory is paused. Conversation can continue without new memories."}</p></div>
      ...
      <div className="memory-toolbar">
        <label className="search-field"><Search aria-hidden="true" /><span className="visually-hidden">Search memories</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search what Luma remembers" /></label>
      ...
      {enabled && visible.length === 0 ? <div className="empty-state"><Search aria-hidden="true" /><h2>No matching memories</h2><p>Try another phrase or add something you explicitly want Luma to remember.</p></div> : null}
      ...
      {adding ? <Modal title="Add a memory" description="Only add something you want used for future continuity." onClose={() => setAdding(false)}><label className="field">Category<select value={type} onChange={(event) => setType(event.target.value as MemoryType)}>{categories.filter((category) => category.value !== "all").map((category) => <option key={category.value} value={category.value}>{category.label}</option>)}</select></label><label className="field">What should Luma remember?<textarea rows={4} value={draft} onChange={(event) => setDraft(event.target.value)} /></label><div className="modal-actions"><button type="button" className="button button--ghost" onClick={() => setAdding(false)}>Cancel</button><button type="button" className="button button--primary" disabled={!draft.trim()} onClick={() => { onAdd(draft.trim(), type); setDraft(""); setAdding(false); }}><Plus aria-hidden="true" /> Add memory</button></div></Modal> : null}
    </section>
  );
}
