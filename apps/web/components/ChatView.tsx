...
export function ChatView({ state, streaming, onSend, onNewConversation, onBack, onCall, onVideoCall, onVoiceNote, onImageUpload, onGenerateImage, onFeedback, onRegenerate, onUpgrade, onCamera }: {
  state: DemoState;
  streaming: boolean;
  ...
  return (
    <section className="workspace chat" aria-labelledby="chat-title">
      ...
      <div ref={messagesViewport} className="chat__messages" aria-live="polite">
        ...
        {messages.map((message) => <MessageBubble key={message.id} message={message} companionName={state.companion.name} onFeedback={onFeedback} onRegenerate={onRegenerate} onWhy={() => setWhyMessage(message)} onReply={() => { setDraft(`Replying to “${message.content.slice(0, 54)}${message.content.length > 54 ? "…" : ""}”\n`); textarea.current?.focus(); }} onEdit={() => { setDraft(message.content); textarea.current?.focus(); }} />)}
...
function MessageBubble({ message, companionName, onFeedback, onRegenerate, onWhy, onReply, onEdit }: { message: ChatMessage; companionName: string; onFeedback: (messageId: string, feedback: "up" | "down", reason?: FeedbackReason) => void; onRegenerate: (messageId: string) => void | Promise<void>; onWhy: () => void; onReply: () => void; onEdit: () => void }) {
  const assistant = message.role === "assistant";
  const [feedbackOpen, setFeedbackOpen] = useState(false);
  const speak = () => { if (!("speechSynthesis" in window)) return; window.speechSynthesis.cancel(); window.speechSynthesis.speak(new SpeechSynthesisUtterance(message.content)); };
