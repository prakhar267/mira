  ...
  try {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (!data) continue;
        const part = JSON.parse(data) as {response?:unknown;error?:unknown;choices?:{delta?:{content?:unknown};finish_reason?:unknown}[]};
        ...
        // OpenAI-shaped Workers streams can finish the model before closing
        // SSE/transport. A successful model stop is authoritative: don't spend
        // the remaining call deadline waiting for a redundant [DONE] frame.
        // Tool calls / truncated generations aren't successful completion.
