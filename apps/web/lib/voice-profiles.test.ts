...
describe("companion voice modes", () => {
  it("keeps one named voice identity while offering emotional delivery modes", () => {
    ...
    expect(new Set(companionVoiceModes.map((mode) => mode.rate)).size).toBeGreaterThan(4);
    expect(companionVoiceModes.every((mode) => mode.rate >= .85 && mode.rate <= 1.05)).toBe(true);
  });
