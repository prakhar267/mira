...
export function playCompanionSpeech(text: string, options: {
  ...
  const playNeuralVoice = async () => {
    ...
    audio.onended = () => end();
    audio.onerror = () => {
      options.onError?.("Cloudflare voice playback failed. Please try again.");
      end();
    };
    await audio.play();
    notifyStart();
  ...
  // Speech is always generated by Cloudflare's free-quota Melo model. Browser
  // voices are intentionally never used, so every device keeps one identity.
  void playNeuralVoice().catch((error) => {
    if (!canceled && !finished) {
      options.onError?.(error instanceof Error ? error.message : "Cloudflare voice is temporarily unavailable.");
      end();
    }
  });
