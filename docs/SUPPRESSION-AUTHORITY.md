...
- Append receipts contain both the reconstructed request-end checkpoint and the current independent head from the same authority transaction. An old valid prefix cannot conceal a later acknowledged deletion. The source must reconstruct the newer range or deny access if its database has rolled back; it cannot jump its local checkpoint to the reported head.
- Missing local protection state is also rejected when an authority is configured outside the source database. Restoring a pre-activation SQL snapshot must not silently select unprotected operation. Dormant operation requires both absent configuration and absent local protection; partial configuration fails closed. Do not set these fields on production before the missing bootstrap/activation protocol is implemented: configuration without enrollment intentionally refuses normal account operations.
...
Further rollback review found two downgrade cases. The regression now physically replaces the synthetic source SQLite file with an earlier copy in which both journal and acknowledgement agree but a subsequently forgotten memory still exists; the retained remote head denies the read. A second case removes the local protection row to simulate a snapshot from before activation and reconstructs the adapter while retaining external authority configuration; normal reads, raw writes and legacy recovery are denied without a network request. Actual Worker-handler tests exercise both boundaries. This does **not** protect simultaneous rollback/removal of all external configuration and local protection, or rollback of the independent authority itself; trusted deployment configuration and authority recovery remain activation requirements.

Fresh local results after these fixes: **336/336** web tests across 42 files; **32/32** Worker tests across six files, including eight new actual-handler scenarios; full web ESLint and route generation/TypeScript passed. The two focused authority/replication files contain 34 tests. These results do not include a new acoustic/device test or live source-loss restore. [CI 34815572597](https://github.com/prakhar267/mira/actions/runs/34815572597) passed the previous commit `4645ed0`, including 90 browser cases, built-artifact smoke and sealed artifact round-trip. That success predates the rollback fixes above; a fresh CI run is required for the final head before merge/promotion.
