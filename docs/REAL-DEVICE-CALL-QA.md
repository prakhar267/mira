...
With opt-in analytics, `call_endpoint_ms`, `call_transcribe_ms`, `call_reply_ms`, `call_speech_ms` and `call_roundtrip_ms` measure the client stages without transcripts or recordings. The microphone adapter supplies elapsed time since its last detected voiced frame; the controller backdates the round-trip start by that endpointing delay and ends at playback's `onStart`. The silence/endpointing window is therefore included. Detection and browser playback events are proxies, not an independent acoustic measurement of the last spoken syllable or audible speaker output. DNT and denied analytics consent suppress these events. Local controller regressions exercise both call labels, not physical devices or different browsers.
