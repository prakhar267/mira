# Mira launch operations — source updated 13 September 2026

This file describes operations for the active `apps/web` source. September 13 changes are not yet deployed. Use [current architecture/configuration](CURRENT-ARCHITECTURE.md), [inference policy](INFERENCE-BOUNDARY.md), [storage/recovery](STORAGE-RECOVERY.md) and [verified release procedure](DEPLOYMENT.md) as the current implementation contract. Historical platform refusals and remote drills below were recorded September 12; they were not rerun here.
...
Follow [deployment and promotion](DEPLOYMENT.md): mandatory unit/Worker/browser checks, the vinext build, isolated execution of built bytes, sealed hashes, explicit authorized promotion and exact deployed SHA/version checks. Post-deploy smoke is now read-only; it does not create accounts or consume provider inference. Historical provider evaluation scripts need updated consent/schema fixtures and an authorized budget before use.

Mira CI/deploy/15-minute external health workflow definitions are included. They require restored GitHub Actions execution and a scoped `CLOUDFLARE_API_TOKEN` plus `CLOUDFLARE_ACCOUNT_ID` in the protected production environment. No such GitHub secrets were present during inspection. CI now checks the committed parser patch and uploads the actual Cloudflare artifact, not a stale Next.js output. A workflow file is not proof that GitHub scheduling or notifications ran. On September 13 the latest health job again had no steps and reported the account payment/spending-limit block. Resolving that account setting is outside this no-payment scope.
